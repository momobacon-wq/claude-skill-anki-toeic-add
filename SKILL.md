...
7. **秀出完整卡片內容**：對每張**成功新增**的卡，用 AnkiConnect `notesInfo` 把 11 個欄位的實際內容抓回來，整理成易讀格式回貼給使用者（讓他不必開 Anki 就能檢視寫進去的字根、例句、搭配、同義字等）。詳見下面「秀出同步後的完整卡片內容」。跳過（重複）的字不必抓。
8. **同步到 Google Doc「單字總表」**：若這次**有成功新增**的字（跳過的不算），把這些新字也追加到使用者固定的可搜尋 Google Doc。詳見下面「同步到 Google Doc 單字總表」。0 張新增就跳過這步。

### 同步到 Google Doc 單字總表（步驟 8）

使用者要求：**每次 /toeic 產生的新單字，都要同步進一份固定的 Google Doc** 方便 Ctrl+F 搜尋。
- 目標 docId：`1-cPrB0z7wJA6jTOR5B2x5Usok2V3zfPHLAEiSfft8m0`（標題「單字」，已含全部歷史卡片，依字母排序）
- 完整原理與檔案說明見 `C:\Users\bacon\anki_doc_sync\README.md`。摘要：Drive API 不能寫既有 Doc，只能用
  **PowerShell 把卡片 HTML 放上 Windows 剪貼簿（CF_HTML，UTF-8 stream）→ Playwright 在瀏覽器按真 Ctrl+V**。

只追加「本次成功新增」的字（用本次成功的 noteId 陣列）：

```powershell
# 1. 用 PowerShell 把本次新增卡的 notesInfo dump 成 json（node 直連 AnkiConnect 會 ECONNRESET，所以走 PowerShell）
$ids = @(1782628865410, 1782628999999)   # ← 換成本次成功新增的 noteId
$b = @{action='notesInfo';version=6;params=@{notes=$ids}} | ConvertTo-Json -Compress
$n = Invoke-RestMethod -Uri http://127.0.0.1:8765 -Method Post -Body $b -ContentType 'application/json'
$n.result | ConvertTo-Json -Depth 8 | Out-File 'C:\Users\bacon\anki_doc_sync\append_dump.json' -Encoding UTF8

# 2. node 生新字的卡片 HTML
cd C:\Users\bacon\anki_doc_sync; node gen-append.mjs append_dump.json

# 3. 把 HTML 放上剪貼簿（一定要 -STA，否則剪貼簿 API 失敗）
powershell.exe -STA -ExecutionPolicy Bypass -File C:\Users\bacon\anki_doc_sync\Set-HtmlClipboard.ps1 C:\Users\bacon\anki_doc_sync\out\append.html
```
然後用 Playwright MCP：
- `browser_navigate` 到 `https://docs.google.com/document/d/1-cPrB0z7wJA6jTOR5B2x5Usok2V3zfPHLAEiSfft8m0/edit`
  （若報 "Browser is already in use" → 先殺含 `mcp-chrome-aeaa311` 的 chrome.exe 殘留再 navigate）
- `browser_run_code_unsafe` 跑 `C:\Users\bacon\anki_doc_sync\paste-end.mjs`（Ctrl+End → Ctrl+V，貼在文末）
- 驗證用**截圖**（Docs 是 canvas，DOM 永遠空；Drive read 顯示中文亂碼只是傳輸問題，doc 本身正確）

回報時最後加一行 `📄 已同步到 Google Doc 單字總表`（或失敗原因）。新字貼在文末（非字母序，Ctrl+F 仍找得到）；
若要全部重新排序，照 README 的「整批重生流程」跑 `gen-html.mjs` + clear-and-paste + 各 chunk。

**前提**：需要 Playwright MCP + 已登入 Google 的 Chrome。若該環境不可用，就略過這步並告訴使用者「Google Doc 同步這次跳過（瀏覽器不可用），卡片已在 Anki」。
...
報告完成後，把**剛剛成功寫進去的每張卡**完整內容抓回來貼給使用者 — 讓他不必開 Anki / 不必等手機同步，就能直接在對話裡檢視所有欄位（字根拆解、3 題型例句、高頻搭配、同義字反義、多益情境等）寫得對不對。

用 `notesInfo` 抓回實際存進 Anki 的欄位值（不是憑記憶複述 — 要抓「真的存進去」的版本）：

```powershell
$body = '{"action":"notesInfo","version":6,"params":{"notes":[1782628865410]}}'  # 換成本批成功的 noteId 陣列
$r = Invoke-RestMethod -Uri http://127.0.0.1:8765 -Method Post -Body $body -ContentType 'application/json'
foreach ($n in $r.result) {
  "noteId : $($n.noteId)　model : $($n.modelName)"
  foreach ($f in $n.fields.PSObject.Properties) {
    "===== $($f.Name) ====="
    $f.Value.value
    ""
  }
...
- **預設一律秀完整版**：把所有 11 個欄位的**全部內容**都呈現出來，等同寫進 Anki 的版本 — 每一條搭配、每一個同義字群、每一句例句**連同它的中譯**都要在，**不要刪減、不要只挑重點摘要**。使用者就是要在對話裡看到跟卡片一模一樣的完整內容。
- **把 raw HTML 轉成可讀的 Markdown** 再呈現（`<h3>` → 小標、`<ul><li>` → 清單、`<blockquote>` → 引言 `>`、`<b>` → 粗體），但**內容一字不少**。短欄位（中文 / English / KK音標 / 發音說明 / 詞性 / Audio / noteId）可用清單或小表格；長欄位（解說 / 例句 / 常用搭配 / 同義字反義 / 多益情境）逐段完整轉寫。不要直接倒未轉換的 raw HTML，但也**不可為了精簡而省略任何一條**。
- **每張成功卡都完整秀**；多張就逐張完整秀，標題用單字本身。即使一次加很多字、即使洗版很長，也照樣每張完整呈現 — 完整性優先於版面長度。
- **跳過（重複）/ 失敗的字不抓**，只秀本次成功新增的。
