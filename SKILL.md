...
- **每張成功卡都秀**；多張就逐張秀，標題用單字本身。
- **整理成易讀格式**（Markdown 表格放短欄位：中文 / English / KK音標 / 發音說明 / 詞性 / Audio / noteId；長欄位用引言或清單摘要 — 解說、例句、常用搭配、同義字反義、多益情境），不要直接倒一大坨 raw HTML 給使用者。重點是讓使用者「掃一眼就看懂卡片長怎樣」。
- **跳過（重複）/ 失敗的字不抓**，只秀本次成功新增的。
- 若使用者**只加 1 個字**，可以秀完整 11 欄；若**一次加很多字**，每張用精簡版（短欄位表格 + 各長欄位 1-2 句重點），避免洗版太長。
